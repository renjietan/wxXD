...
const app = getApp(); Page({
  ...
  formsubimt() {
    ...
    if (phone == '' || phone == ' ' || phone == null) {
      ...
      wx.request({
        url: app.globalData.serverEncry + 'WeChatBingMsg',
        method: 'POST',
  ...
  getmsg(random, phone) {    
    wx.request({
      url: app.globalData.serverEncry + 'sendbangmsg',
      method: 'POST',
      data: {
        "random": random,
        "telephone": phone
  ...
  onLoad: function (options) {
    ...
    //https://cj.datasurge.cn:9089/wx/Third/saveSessionId.do
    wx.request({
      url: app.globalData.serverEncry + '/wx/Third/saveSessionId.do',
      method: 'POST',
      success: (result) => {
        //wx.setStorageSync('sessionId', result.data.sessionId);
        this.sessionId = result.data.sessionId;
        wx.request({
          url: app.globalData.serverEncry + 'BindRegister',
          method: 'POST',
          header: {
          ...
          success: (result) => {
            ...
            // wx.navigateTo({
            //   url: '../register/register?random=' + random
            // })
          },
          fail: function ({
            errMsg
