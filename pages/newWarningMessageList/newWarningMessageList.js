...
const app = getApp(); Page({
  ...
  getMsg(pages, startTime, endTime, isrefresh) {
    ...
    $request({
      url: app.globalData.serverWJ + "wx/user/selectExprienceWarningMsg.do", method: "POST",
      params:{
        "page": pages,
        "size": "7",
        "startTime": startTime,
      ...
      if (result.data.resultCode == 0) {
        if (!isrefreshs) {
          ...
          let news = newarr.concat(result.data.warnMsgList);
          ...
          let newarr = result.data.warnMsgList;
          //console.log(newarr)
          this.count = result.data.totalCount;
          this.setData({
            warnMsgList: newarr,
